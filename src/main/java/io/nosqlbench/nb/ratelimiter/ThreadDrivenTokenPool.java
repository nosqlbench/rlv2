...
public class ThreadDrivenTokenPool implements TokenPool {
    private static final boolean amortize_here = true;
     ...
     */
    public ThreadDrivenTokenPool(final RateSpec rateSpec, final Map<String, String> named, final int amortization) {
    ...
    public synchronized TokenPool apply(final Map<String, String> labeled, final RateSpec rateSpec) {
        ...
        maxActivePool = Math.max((long) 1.0E6,
                (long) (rateSpec.getNanosPerOp() * ThreadDrivenTokenPool.MIN_CONCURRENT_OPS));
    ...
    public synchronized long blockAndTake() {
        ...
        while (this.activePool < this.nanosPerOp) {
            this.blocks++;
            // System.out.println(ANSI_BrightRed + "waiting for " + amt + "/" + activePool +
            // " of max " + maxActivePool + ANSI_Reset);
            try {
                this.wait(1000);
        ...
        // System.out.println(ANSI_BrightYellow + "taking " + amt + "/" + activePool +
        // ANSI_Reset);
