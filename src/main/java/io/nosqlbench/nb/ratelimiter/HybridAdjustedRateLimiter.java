 ...
 *
 * <H2>Design Notes</H2>
 * This implementation makes certain trade-offs needed to support a combination
 * of requirements.
 * Specifically, some small degree of inaccuracy is allowed to enable higher
 * throughput when
 ...
 * <LI>Controlling the accuracy of a delay is not possible under any level of
 * load.</LI>
 * <LI>It is undesirable (wasteful) to use spin loops to delay.</LI>
 * </OL>
 *
 * Together, these factors mean a compromise is inevitable. In practice it means
 * that a very accurate
 * implementation will likely be very slow, and a very fast implementation will
 * likely be very inaccurate.
 * This implementation tries to strike a balance, providing accuracy near the
 * microsecond level,
 * while allowing rates in the tens of millions per second, even under heavy
 * thread contention.
 *
 * <H2>Burst Ratio</H2>
 * <p>
 * This rate limiter provides a sliding scale between strict rate limiting and
 * average rate limiting,
 * the difference between the two controlled by a <em>burst ratio</em>
 * parameter. When the burst
 * ratio is 1.0, the rate limiter acts as a strict rate limiter, disallowing
 * faster operations
 * from using time that was previously forfeited by prior slower operations.
 * This is a "use it
 * or lose it" mode that means things like GC events can steal throughput from a
 * running client
 * as a necessary effect of losing time in a strict timing sense.
 * </p>
 *
 * <p>
 * When the burst ratio is set to higher than 1.0, faster operations may recover
 * lost time from
 * previously slower operations. This means that any valleys created in the
 * actual op rate of the
 * client can be converted into plateaus of throughput above the strict rate,
 * but only at a speed that
 * fits within (op rate * burst ratio). This allows for workloads to approximate
 * the average
 * target rate over time, with controllable bursting rates. This ability allows
 * for near-strict
 * behavior while allowing clients to still track truer to rate limit
 * expectations, so long as the
 * overall workload is not saturating resources.
 * </p>
 */
@Service(value = RateLimiter.class, selector = "hybrid")
public class HybridAdjustedRateLimiter implements RateLimiter {
    private static final boolean amortize_here = false;
    private Map<String, String> named;

    // private volatile TokenFiller filler;
    private volatile long starttime;

    private final int amoritzation_steps;
    private static final ThreadLocal<Integer> amortization_count = new ThreadLocal<Integer>() {
        @Override
    ...
    // TODO Doc rate limiter scenarios, including when you want to reset the
    // waittime, and when you don't
    private final AtomicLong cumulativeWaitTimeNanos = new AtomicLong(0L);

    protected HybridAdjustedRateLimiter() {
        amoritzation_steps = 1;
    }

    public HybridAdjustedRateLimiter(final Map<String, String> named, final String label, final RateSpec rateSpec,
            final int amoritzation_steps) {
        this.label = label;
        this.amoritzation_steps = amoritzation_steps;
        this.init(named);
        this.named = named;
    ...
    public long maybeWaitForOp() {
        if (amortize_here) {
            final int amortized = amortization_count.get();
            if (amortized == amoritzation_steps) {
                amortization_count.set(0);
                return this.tokens.blockAndTake();
            } else {
                amortization_count.set(amortized + 1);
                return 0;
            }
        } else {
            return this.tokens.blockAndTake();
    ...
    public synchronized void applyRateSpec(final RateSpec updatingRateSpec) {
        ...
        if (null == updatingRateSpec)
            throw new RuntimeException("RateSpec must be defined");

        if (updatingRateSpec.equals(rateSpec) && !updatingRateSpec.isRestart())
            return;

        rateSpec = updatingRateSpec;
        tokens = null == this.tokens ? new ThreadDrivenTokenPool(this.rateSpec, this.named, this.amoritzation_steps)
                : tokens.apply(this.named, this.rateSpec);

        if ((State.Idle == this.state) && updatingRateSpec.isAutoStart())
            start();
    ...
    public synchronized void start() {
        ...
        switch (this.state) {
            case Started:
                // logger.warn("Tried to start a rate limiter that was already started. If this
                // is desired, use restart() instead");
    ...
    public String toString() {
        ...
        sb.append("{\n");
        if (null != this.getRateSpec())
            sb.append("      spec:").append(rateSpec.toString());
        if (null != this.tokens)
